...
for (const page of pages) {
  ...
  if (meta(html, "property", "og:image") !== `${siteUrl}/assets/og-image.png`) fail(page.file, "og:image must use the 1200x630 shared image");
  ...
  if (!meta(html, "name", "twitter:card")) fail(page.file, "missing twitter:card");
  if (!meta(html, "name", "robots")) fail(page.file, "missing robots meta");
...
console.log(`Site audit passed: ${pages.length} indexable pages, unique metadata, valid JSON-LD, breadcrumbs, internal links and sitemap.`);
