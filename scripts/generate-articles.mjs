...
// 1ファイル＝1記事、ファイル名がそのままURL（/articles/{ファイル名}）になる。
// 出力ファイルは articles/{ファイル名}.html だが、Cloudflare Pages が拡張子なしで配信するため、
// canonical・内部リンク・sitemapはすべて拡張子なしに統一する。
...
// 記事本文はMarkdownで手書きするため、リンクが ../childrens-shoes.html のように
// 拡張子つきで書かれることがある。公開URLは拡張子なしに統一しているので、
// サイト内リンクだけ .html を落とす（外部リンクとファイルへのリンクは触らない）。
function toExtensionlessLinks(html) {
  return html.replace(/href="((?:\.{1,2}\/)[^":]*?)\.html(#[^"]*)?"/g, 'href="$1$2"');
}
...
async function loadArticles() {
  ...
  for (const fileName of fileNames.sort()) {
    ...
    articles.push({
      ...
      article_content: toExtensionlessLinks(markdownToHtml(body)),
...
`;
  ...
  // 記事が1本も無いあいだは sitemap-articles.xml が空になる。空のsitemapを
  // robots.txt で案内すると Search Console が「サイトマップが空です」で
  // エラーにするため、記事の有無に合わせて案内行を出し入れする。
  const robotsUrl = new URL("robots.txt", root);
  const robotsLine = `Sitemap: ${siteUrl}/sitemap-articles.xml`;
  const robots = await readFile(robotsUrl, "utf8");
  const hasLine = robots.includes(robotsLine);
  if (validArticles.length && !hasLine) {
    await writeFile(robotsUrl, robots.replace(`Sitemap: ${siteUrl}/sitemap.xml\n`, `Sitemap: ${siteUrl}/sitemap.xml\n${robotsLine}\n`), "utf8");
  } else if (!validArticles.length && hasLine) {
    await writeFile(robotsUrl, robots.replace(`${robotsLine}\n`, ""), "utf8");
  }
